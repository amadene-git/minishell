...
t_tok	*create_tok_arg(const char *str)
{
	int i;
	int j;
	t_tok *token = NULL;
	...
	while (str[i])
	{
		...
		else if (str[i])
		{
			...
			while (str[j] && str[j] != ' ')
				j++;
			if (str[i] && j > i)
				token = create_tok(CHR_ST, ft_strndup(str + i, j - i));
			else
				token = NULL;
			i = j;	
		}
	...
	while (str[i])
    {
        ...
        else if (str[i] == '$')
        {
            ...
            while (ft_isalpha(str[j]) || ft_isdigit(str[j]) || str[j] == '_')
                j++;
            ptr = ft_strndup(str + i + 1, j - (i + 1));
			if ((elem = dlist_chr(cmd->envlist, ptr)) && elem->data->value && elem->data->value[0])
			{
            	token_push_back(&token, create_tok_arg(elem->data->value));
				i += ft_strlen(elem->data->value) - 1;
			}
			free(ptr);
		}
	...
	while ((*tok_lex)->type > CHR_ERROR && (*tok_lex)->type < CHR_OP)
		{
			...
			else if ((*tok_lex)->type == CHR_WORD)
			{
				...
				if (token && !token_push_back(&cmd->tok_arg, token))
					return (NULL);
			}
			tok_lex++;
		}
